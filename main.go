...
func main() {
	...
	savecode := func(p *Element) {
		...
		switch lang {
		case "language-html":
			pre = mkcodepen(code.GetContent(), "", "", pre)
		case "language-css":
			pre = mkcodepen(html.EscapeString("<html><head></head><body><h1>heading</h1><p>some text</p></body></html>"), code.GetContent(), "", pre)
		case "language-js":
			pre = mkcodepen("", "", code.GetContent(), pre)
		}
